...
export function renderEnvironmentLine(ctx) {
    ...
    if (display?.showPluginCount && ctx.pluginCount > 0) {
        parts.push(`${ctx.pluginCount} ${t("label.plugins")}`);
    }
    if (display?.showSkillCount && ctx.skillCount > 0) {
        parts.push(`${ctx.skillCount} ${t("label.skills")}`);
    }
