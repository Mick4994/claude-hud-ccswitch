...
> **关于本 fork**：本仓库是 [`jarrodwatts/claude-hud`](https://github.com/jarrodwatts/claude-hud) 的派生版本，专为 CC Switch 代理用户设计。
>
> - 上游 release note 保留以追根；从 `[Fork]` 起的条目才是本仓库独立变更。
> - 本 fork 的版本号独立维护（参见 `.claude-plugin/plugin.json`）。

## [Fork] - 2026-07-04

### Added
- 新模块 `src/ccswitch.ts`：读取 `~/.cc-switch/settings.json` 与 `~/.cc-switch/cc-switch.db`，解析当前激活 provider 的 `ANTHROPIC_DEFAULT_*_MODEL[_NAME]`，返回真实上游模型名。
- 在 `src/stdin.ts::getModelName()` 中检测代理伪装标签（`/^claude-/`）并替换为真实模型；保留所有上游功能（autoCompactWindow、advisor、externalUsage 等）。
- 新文件 `wrapper/statusline.mjs`：纯 `import.meta.url` 反查 dist 路径，零硬编码路径，跨平台。
- 支持 `CCSWITCH_DIR`、`CCSWITCH_SQLITE3_PATH` 两个环境变量覆盖。
- 新增 `tests/ccswitch.test.js`：覆盖 `isProxyLabel` 边界与 `resolveModelFromConfig` 各 tier 解析。

### Changed
- `src/ccswitch.ts`：移除 `A:\\Program Files\\sqlite\\sqlite3.exe` 硬编码候选；改用 `process.env.ProgramFiles`。
- 包名 `claude-hud` → `claude-hud-ccswitch`（v0.2.0）。
