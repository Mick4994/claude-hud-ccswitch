...
import { getCCSwitchModel, isProxyLabel } from './ccswitch.js';
...
export function getModelName(stdin: StdinData): string {
  ...
  if (displayName) {
    // CC Switch (and similar proxies) report the Claude-native alias in
    // display_name while routing to a different upstream model. When we can
    // read the real model from CC Switch, prefer that so the HUD badge matches
    // the model the user is actually being billed for.
    if (isProxyLabel(displayName)) {
      const realModel = getCCSwitchModel(stdin.model?.id);
      if (realModel) {
        return realModel;
      }
    }
