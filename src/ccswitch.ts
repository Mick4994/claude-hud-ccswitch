...
// ponytail: 路径与环境变量分层 —— CCSWITCH_DIR → home + '.cc-switch'，db 文件名不可改
// ponytail: sqlite3 走「环境变量 → 通用候选 → PATH which」三段
// 上游 ccswitch 用户安装路径非常规（git clone / scoop / appimage），所以必须支持覆盖
...
function getCCSwitchDir(): string {
...
function findSqlite3Executable(): string | null {
  ...
  if (process.platform === "win32") {
    ...
    candidates.push(
      path.join(getHomeDir(), ".local", "bin", "sqlite3"),
      "/usr/bin/sqlite3",
...
function readCurrentProviderId(): string | null {
  const settingsPath = path.join(getCCSwitchDir(), "settings.json");
  try {
    const raw = fs.readFileSync(settingsPath, "utf-8");
    const settings = JSON.parse(raw) as CCSettings;
...
function queryProviderConfig(providerId: string): CCProviderConfig | null {
  ...
  try {
    ...
    return JSON.parse(output.trim()) as CCProviderConfig;
  } catch {
    return null;
  }
}
...
export function resolveModelFromConfig(config: CCProviderConfig, claudeModelId?: string): string | null {
  ...
  const nameKey = `ANTHROPIC_DEFAULT_${tierUpper}_MODEL_NAME` as const;
  const modelKey = `ANTHROPIC_DEFAULT_${tierUpper}_MODEL` as const;
 ...
 */
export function getCCSwitchModel(claudeModelId?: string): string | null {
  const dbPath = getCCSwitchDbPath();
 ...
 */
export function isProxyLabel(name: string): boolean {
