...
function collectSkillDirs(dir: string, out: Set<string>): void {
  ...
  try {
    ...
    debug(`Failed to read skills from ${dir}:`, error);
  }
}

function getSkillCount(claudeDir: string): number {
  const skillDirs = new Set<string>();
  collectSkillDirs(path.join(claudeDir, 'skills'), skillDirs);
  collectSkillDirs(path.join(claudeDir, 'plugins', 'cache'), skillDirs);
  return skillDirs.size;
}

function getEnabledPluginCount(claudeDir: string): number {
  ...
  try {
    ...
    debug(`Failed to read plugins from ${installedPath}:`, error);
  }
  return 0;
}
...
function isConfigCounts(value: unknown): value is ConfigCounts {
  ...
  return (
    ...
    && counts.hooksCount >= 0
    && typeof counts.pluginCount === 'number'
    && Number.isFinite(counts.pluginCount)
    && counts.pluginCount >= 0
    && typeof counts.skillCount === 'number'
    && Number.isFinite(counts.skillCount)
    && counts.skillCount >= 0
...
function computeConfigCountsFresh(cwd?: string): ConfigCounts {
  ...
  const pluginCount = getEnabledPluginCount(claudeDir);
  const skillCount = getSkillCount(claudeDir);

  return { claudeMdCount, rulesCount, mcpCount, hooksCount, pluginCount, skillCount, outputStyle };
}
